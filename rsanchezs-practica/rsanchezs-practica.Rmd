...
LLavors tenim que en el conjunt de dades `tickets_client` existeixen 123 valors extrems:
...
tickets_client <- tickets_client %>% 
        mutate(TIPUS = case_when(between(TOTALCOMPRES, 1, 5) ~ "OCASIONAL",
                                  between(TOTALCOMPRES, 6, 21) ~ "FREQUENT",
...
tickets_client %>% 
  group_by(GENERE) %>% 
   count(TIPUS) %>% 
    ...
    ggplot(mapping = aes(x = GENERE, y = n, fill = TIPUS)) +
...
Avançant en el nostre anàlisi exploratori , passem a examinar la covariació entre les variables `REGIO` i `TIPUS`:
...
Un altre punt important, sería estudiar si existeix correlació entre les variables de tipus .

En primer lloc, passem a analitzar la 



```{r}
tickets_client %>% ggplot(mapping = aes(x=EDAT, y=TOTAL, color=GENERE)) +

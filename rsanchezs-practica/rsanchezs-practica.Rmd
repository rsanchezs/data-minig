...
Recollint tot el que s´ha realitzat, tenim 11 atributs per a la nostra anàlisi i 923 observacions en el conjunt de dades `client`:
...
pren els valors entre 1 i 48 dividint els valors entre els conjunts: __FREQÜENT__, __HABITUAL__ i __OCASIONAL__.
...
```{r}
tickets_client <- tickets_client %>% 
        mutate(TIPUS = case_when(between(TOTALCOMPRES, 1, 5) ~ "OCASIONAL",
                                  between(TOTALCOMPRES, 6, 16) ~ "FREQUENT",
                                  between(TOTALCOMPRES, 17, 48) ~ "HABITUAL" ))

# Canviem la variable de tipus `character` a `factor`
tickets_client$TIPUS <- as.factor(tickets_client$TIPUS)
```
...
En primer lloc, passem a estudiar la variable `GENERE` respecte a `TIPUS`. Per tal d´explorar la relació entre aquestes variables realitzarem les següents operacions:

- Agrupar el conjunt de dades per la variable `GENERE`.
- Contar el nombre d´observacions de la variable `TIPUS` que apareixen en cada classe del atribut `GENERE`.
- Calcular el percentatge en cada classe segons la variable `TIPUS`.
...
tickets_client %>% 
  group_by(GENERE) %>% 
   count(TIPUS) %>% 
    ...
    ggplot(mapping =  aes(y = n, x = GENERE, color=TIPUS, fill=TIPUS)) + 
...
tickets_client %>% 
  group_by(ESTATCIVIL) %>% 
   count(TIPUS) %>% 
    mutate(freq = round(n / sum(n) * 100, 0)) %>% 
    ggplot(mapping = aes(x = ESTATCIVIL, y = n, fill = TIPUS)) +
    geom_bar(stat = "identity") +
...
tickets_client %>% 
  group_by(PROFESSIO) %>% 
   count(TIPUS) %>% 
    mutate(freq = round(n / sum(n) * 100, 0)) %>% 
    ggplot(mapping = aes(x = PROFESSIO, y = n, fill = TIPUS)) +
    geom_bar(stat = "identity") +
    geom_text(aes(label = paste(freq, "%"))) +
...
Avançant en el nostre anàlisi exploratori , passem a examinar el percentatge de variació entre les variables `REGIO` i `TIPUS`:
...
tickets_client %>% 
  group_by(REGIO) %>% 
   count(TIPUS) %>% 
    mutate(freq = round(n / sum(n) * 100, 0)) %>% 
    ggplot(mapping = aes(x = REGIO, y = n, fill = TIPUS)) +
    geom_bar(stat = "identity") +
    geom_text(aes(label = paste(freq, "%"))) +
...
tickets_client %>% 
  group_by(NACIONALITAT) %>% 
   count(TIPUS) %>% 
    mutate(freq = round(n / sum(n) * 100, 0)) %>% 
    ggplot(mapping = aes(x = NACIONALITAT, y = n, fill = TIPUS)) +
    geom_bar(stat = "identity") +
    geom_text(aes(label = paste(freq, "%"))) +
    theme(axis.text.x = element_text (size = 14, angle=45)) +
    facet_wrap(~TIPUS)
...
```{r echo=FALSE}
tickets <- tickets_client[, c(8, 10, 12)]
...
```{r echo=FALSE}
sample <- tickets_client %>% 
                  ungroup() %>% 
                  sample_frac(size = 0.05, replace = TRUE)
tickets <- scale(sample[, c(8, 10, 12)])
```
...
inTrain <- createDataPartition(tickets_client$TIPUS, p = 0.8, list = FALSE)
...
```{r echo=FALSE}
library(rpart)
modelFitCART <- rpart(TIPUS ~ ., data = training, method = "class")
```
...
Finalment, obtenim els indicadors de rendiment del model amb el conjunt de prova i els guardem al dataframe `Predictions`:
...
TabCART = table(training$TIPUS, PredictCART)
...
modelFitRandomForest <- randomForest(y = training$TIPUS,
                                     x = training[, -4])
```
...
Per concloure, de forma similiar als apartats anteriors obtenim els valors de les mesures d´avaluació del model:
...
no és suficient, de manera que seria recomanable no utilitzar cap d'aquests algoritmes per a la predicció del model.

...
output:
  word_document:
    highlight: zenburn
    reference_docx: word-styles-reference-26.docx
...
c) Podríeu donar, tres exemples de regles que es poguessin derivar de l’arbre de decisió del vostre projecte?
...
__* Conjunt d´entrenamet:__ Un subconjunt per a entrenar el model.
__* Conjunt de prova:__ Un subconjunt per a provar el model entrenat.
...
### Classificació amb C4.5 sense poda [^18]
...
### Classificació amb C4.5 amb poda
...
library(RWeka)
# Utilitzem el model c4.5 amb poda
...
Per acabar, de forma similar al que hem fet anteriorment classificarem el conjunt de prova i assignarem les mesures d´avaluació a `Predictions`:
...
Finalment, obtenim les mesures d´avaluació del model amb el conjunt de prova i les guardem al dataframe
`Predictions`:
...
# Carreguem el framework `caret`
library(caret)
# Utilitzem l´algoritme random forest
...
Amb el segúent fragment de codi analitzarem quin algoritme ha obtingut millor
prediccions en el conjunt de dades de prova:

```{r collapse=FALSE}
# Examinen les prediccions del conjunt de prova
